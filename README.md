...
├── tests/
│   └── test_reproduction.py          22 checks (pytest or standalone)
├── data/
...
└── docs/
    ├── REPRODUCTION_REPORT.md        full scope map, per-result table, parameter provenance
    └── DOCUMENT_SEARCH_LIST.md       which source documents pin which parameters
```
...
(details in [`docs/REPRODUCTION_REPORT.md`](docs/REPRODUCTION_REPORT.md) §6):
...
`docs/DOCUMENT_SEARCH_LIST.md` maps each remaining parameter to the source document that
pins it, for anyone extending the reproduction.
...
- **Simulation** — the paper's own simulations, re-implemented on independent code:
  unitary/non-unitary fidelity, Iris ~94.67%, PUF ~50%/~50%, ring solver, switch crosstalk.
...
- Directional-coupler dispersion, propagation loss, grating-coupler references and the
  provenance of each borrowed parameter are listed in `docs/REPRODUCTION_REPORT.md` §4.2
  and `docs/DOCUMENT_SEARCH_LIST.md`.
