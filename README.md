...
| Switch crosstalk (Fig 4d,e) | −45 to <−20 dB | cross state, mesh model fitted to Fig 4d: -27.2 to -21.5 dB at 1560 nm; worst -17.0 dB over the fitted 1549–1565 nm, -11.9 dB extrapolated over 1530–1549 nm. Bar state, coupler-split spread fitted to Fig 4e: -107.8 to -31.2 dB at 1560 nm, RMS 0.42 dB over 27 digitized points, from a fitted spread of **0.0182** whose full range across the percentile scan and the calibration offset test is **0.0152 to 0.0707** — that range, not the much narrower bootstrap interval, is the honest uncertainty; the arm-phase spread is still assumed (report §6.1) | fit to measurement |
...
not a guessed op-count — see [`docs/REPRODUCTION_REPORT_v9.md`](docs/REPRODUCTION_REPORT_v9.md) §6.
...
├── tests/
│   └── test_reproduction.py          52 checks (pytest or standalone)
├── data/
...
└── docs/
    ├── REPRODUCTION_REPORT_v9.md     full scope map, per-result table, parameter provenance
    ├── REPRODUCTION_REPORT_v8.md     the previous report
    ├── REPRODUCTION_REPORT_v7.md     the report before that
    ├── REPRODUCTION_REPORT_v6.md     earlier reports, kept for history
...
python scripts/fit_fig4e.py      # → bar-state coupler-split spread 0.0182 (range 0.0152-0.0707), RMS 0.42 dB over 27 points; writes figures/fig4e_digitized.png
```
...
(details in [`docs/REPRODUCTION_REPORT_v9.md`](docs/REPRODUCTION_REPORT_v9.md) §6). The
parameters that are not from the paper, with their sources where recorded, are listed in
docs/REPRODUCTION_REPORT_v9.md §6.3:
...
- bar-state coupler-split spread **fitted to the digitized Fig 4e crosstalk**
  (`scripts/fit_fig4e.py`), **0.0182** with a full range of
  **0.0152 to 0.0707**; the arm-phase spread is still assumed, because one curve
  constrains only the combination of the two

The superseded document search list is kept for history only; current parameter
provenance is in docs/REPRODUCTION_REPORT_v9.md §6.
...
- Directional-coupler dispersion, propagation loss, grating-coupler references and the
  ...
  [`docs/REPRODUCTION_REPORT_v9.md`](docs/REPRODUCTION_REPORT_v9.md) §6.3. The superseded
  document search list is kept for history only; current parameter provenance is in
  docs/REPRODUCTION_REPORT_v9.md §6.
