...
| 778, 707, 71, 74 | Status | how many document numbers and source-line citations the suite checks. Counts of the checks themselves, reported by the four document tests, not values `results.json` holds |
...
| 0.8 dB per coupler; four, six and eight couplers per path; −4.20 and −7.40 dB; 4.41 dB | §7.2 | the excess loss the cited source gives, the number of directional couplers an intended path crosses in the four-stage fabric, and what the modelled on-chip loss would be at that excess loss. The first is another paper's measurement; the rest are arithmetic on it and on the module constants, computed for the report and not stored in `results.json` |
| the values quoted from cited sources in the §6.3 classifications and in `docs/CITATION_CHECK.md` — 2.14, 2.2 ± 0.8, 2.4 ± 0.3, 19 dies, 4.4 ± 0.2, 45, 1545, 0.8, 0.599, 0.820, 0.36, 0.67, 38 | §6.3, §7.3 | numbers this repository reads out of other people's papers, each with its location and a quotation in `docs/CITATION_CHECK.md` and the fetched source under `docs/citation_check/raw/`. They are other people's measurements, not outputs of this pipeline, so `results.json` cannot hold them |
