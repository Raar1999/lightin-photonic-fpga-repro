...
| 23r | PUF uniqueness, recirculating mesh, 100 dies | **49.97%** (simulation) | **49.97%** (wiring C4_FREE_1), **49.99%** (C4_FREE_2) | **model vs simulation** |
| 24r | PUF uniformity, recirculating mesh, 100 dies | **50.15%** (simulation) | **49.32%** (C4_FREE_1), **49.39%** (C4_FREE_2) | **model vs simulation** |
| 25r | PUF reliability, recirculating mesh | 2.55% intra-die HD (experimental) | **0.73%** (C4_FREE_1), **0.75%** (C4_FREE_2) | **model vs measurement** |
...
At the arm-length-derived spread the feed-forward model gives uniqueness 48.92%,
uniformity 49.39%, intra-die reliability 0.74% and a tie fraction of 0.0.
...
At the arm-length-derived spread, over 100 dies and 128 challenges:

| | uniqueness | uniformity | reliability (intra-die HD) |
|---|---|---|---|
| paper, 100-die simulation | **49.97%** | **50.15%** | 2.55% (experimental, 2 dies) |
| recirculating, C4_FREE_1 | **49.97%** | 49.32% | 0.73% |
| recirculating, C4_FREE_2 | **49.99%** | 49.39% | 0.75% |
| feed-forward | 48.92% | 49.39% | 0.74% |

On uniqueness the recirculating model is the closer of the two, and it is closer by a wide
margin: 49.97% against the paper's 49.97%, a gap of 0.00 points, where the feed-forward
model sits 1.05 points away. On uniformity the two are barely distinguishable -- 49.32%
against 49.39%, both about 0.8 points below the paper's 50.15% -- and on reliability they
differ by 0.01 points, with the paper's 2.55% not comparable in any case, being an
experimental two-die number against a model driven by an assumed measurement noise.

Whether the difference between the two models is larger than the difference between either
model and the paper depends on which metric is asked. On uniqueness it is: the two models
differ by 1.06 points, which is larger than the recirculating model's 0.00-point gap to
the paper and about the same as the feed-forward model's 1.05-point gap. On uniformity it
is not: the models differ by 0.06 points while both sit about 0.8 points from the paper, so
there the choice of topology is the smaller effect. Nothing here was tuned; the
recirculating model has no free parameter that the feed-forward one lacks.
...
The two wirings agree on uniqueness to 0.0002 against a stated tolerance of 0.05, which is
the check that the conclusion does not rest on the wiring. A test enforces it.
...
* Digitizing the paper's Fig 4e bar-state spectra would constrain the bar-state model
  against measurement, in the same way Fig 4d constrains the cross state, and would replace
  the two assumed fabrication spreads with fitted values. `docs/FIG4E_SCOPE.md` sets out what
  that would involve.
