...
| 17b | Switch crosstalk at 1560 nm, bar state (Fig. 4e) | −45 to <−20 dB | bar **−107.80 to −31.16 dB**, from a coupler-split spread fitted to Fig 4e and an arm-phase spread still assumed (§6.1, §6.3) | **fit to measurement** |
...
| 20e | Bar-state T32 model vs digitized Fig 4e | — | RMS **0.42 dB** over 27 points | **fit to measurement** |
...
`SIGMA_SPLIT` = 0.0182 comes from the digitized Fig 4e curve (§6.1), while `SIGMA_PHASE`
is still 0.02 rad with no recorded source (§6.3). Set both to zero and the bar state nulls
exactly, below the 1e-20 structural-zero threshold. Leave them and the cell leaks
−30.20 dB (`bar_cell_leak_db_model`); ideal 50:50 couplers, which remove the split
imbalance but leave the phase error, give −32.87 dB
...
tick marks per axis with sub-pixel residuals rather than to figure-read endpoints. As an
axis check, the T22 through path in the same sub-panel reads −1.40 to −2.35 dB against the
paper's measured on-chip insertion loss of −1.85 to −2.99 dB.
...
|---|---|---|---|---|
| coupler-split spread | `SIGMA_SPLIT` | `SIGMA_PHASE` = 0.02 | **0.0182 ± 0.0002** | **0.42 dB** |
...
Bootstraps on the one well-posed fit, 500 resamples each, seeds 0 and 1, matching the
Fig 4d procedure: pairs [0.0179, 0.0185] (0 failed fits), parametric at the CSV's
±0.3 dB [0.0180, 0.0184] (0 failed). Both intervals are far narrower than the
spread's real uncertainty, which is dominated by two things the bootstrap does not see: the
ensemble seed (0.16 dB at 3200 realisations) and the choice of percentile (0.0283 at the
90th, 0.0152 at the 99.9th, at the same 0.42 dB RMS).
...
refitted, the Fig 4d mesh model scores 0.79 dB RMS on its own 25 points, against
0.82 dB at the 0.02 spread it carried before. Adopting the Fig 4e result did not
degrade the Fig 4d agreement; it improved it by 0.03 dB. The Fig 4e model scores 0.42 dB
on its 27 points, against 0.90 dB at the assumed spread. These are the `cross_check`
keys in `results.json`. (Figure: `figures/fig4e_digitized.png`.)
...
| `SIGMA_SPLIT` | 0.0182, clipped to [0.3, 0.7] | `switching.py`; bar-state crosstalk | §6.1 — fit of `bar_model` to the 27 points of `data/fig4e_bar_digitized.csv`, digitized from the paper's Fig 4e; parametric bootstrap [0.0180, 0.0184] |
...
* `python -m pytest -q` takes about 22 seconds on this machine, because
  ...
  `python scripts/run_all.py` takes about 25 minutes, of which the Fig 4e bootstrap is
  roughly a third; `python scripts/run_all.py --quick` runs the same pipeline in about
  6 minutes with the Iris seed sweep cut to two seeds and every Fig 4d and Fig 4e bootstrap
  to 50 resamples, writing `results_quick.json` and `figures_quick/`
...
* The comparison statistic for Fig 4e is a modelling choice. The digitized points are the
  band's upper envelope, so the model is compared at the 99th percentile of its
  fabrication ensemble; at the 90th the fitted spread is 0.0283 and at the 99.9th it is
  0.0152, and the RMS is the same 0.42 dB at all three, so the data cannot choose between
  them. The percentile is set by how many independent ripple samples fall inside one
  digitization window, which the figure does not show.
